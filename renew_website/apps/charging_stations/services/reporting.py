...
from .exports import (
    ...
    get_transaction_summary_bucket,
...
def _build_charger_status_counts(connectors) -> dict:
    ...
    return charger_status_counts


def build_quick_stats(transactions) -> dict:
    tx_list = list(transactions)
    classifications = [classify_transaction(tx) for tx in tx_list]
    return {
        "successful": sum(1 for item in classifications if item["summary"] == "successful"),
        "partial": sum(1 for item in classifications if item["summary"] == "partial"),
        "failed": sum(1 for item in classifications if item["summary"] == "failed"),
        "simulated": sum(1 for item in classifications if item["summary"] == "simulated"),
...
def build_public_dashboard_context(request) -> dict:
    ...
    charger_status_counts = _build_charger_status_counts(connectors)
...
def build_dashboard_live_summary_payload(request) -> dict:
    now = timezone.now()
    today = now.date()

    stations = list(Station.objects.all())
    connectors = list(Connector.objects.select_related("station").all())
    transactions = Transaction.objects.select_related("billing")
    today_transactions = transactions.filter(started_at__date=today)
    charger_status_counts = _build_charger_status_counts(connectors)
...
def build_session_chart_payload(request) -> dict:
    ...
    for tx in txs:
        ...
        if label in buckets_map:
            summary = get_transaction_summary_bucket(tx)
